 ...
 * narmesteLederId is needed to provide correct redirect URL for NL users.
 */
const validateAndGetIdPortenTokenOrRedirect = async (
  narmesteLederIdIfAG: string | false
) => {
  const validationResult = await validateIdPortenToken();

  if (!validationResult.success) {
    return narmesteLederIdIfAG
      ? redirectToLoginForAG(narmesteLederIdIfAG)
      : redirectToLoginForSM();
  }
...
export async function tokenXFetchGet<S extends z.ZodType>({
  ...
  const idPortenToken =
    await validateAndGetIdPortenTokenOrRedirect(narmesteLederIdIfAG);
