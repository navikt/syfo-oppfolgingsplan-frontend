...
import { OversiktSideIntroduksjonForSykmeldt } from "@/components/OversiktSide/InformasjonSection/OversiktSideIntroduksjon";
import {
  DetteKanDuBidraMed,
  ForberedelseTilSamtale,
} from "@/components/OversiktSide/InformasjonSection/SykmeldtTiltaksgruppeInformasjon";
...
export default async function PlanListeForSykmeldt() {
  ...
  const harGjeldendeUnntak = gjeldendeHendelser.some(
    ({ hendelse }) => hendelse.type === "PLAN_IKKE_NODVENDIG",
  );
  const visForberedelseForTomtilstand =
    erITiltaksgruppe && gjeldendeHendelser.length === 0;

  return (
    <>
      <OversiktSideIntroduksjonForSykmeldt
        erITiltaksgruppe={erITiltaksgruppe}
      />
      {erITiltaksgruppe && <DetteKanDuBidraMed />}

      <section className="mb-8">
        {gjeldendeHendelser.length === 0 && (
          <IngenAktivPlanAlert erITiltaksgruppe={erITiltaksgruppe} />
        )}
        {visForberedelseForTomtilstand && (
          <ForberedelseTilSamtale harGjeldendeUnntak={false} />
        )}
        {gjeldendeHendelser.length > 0 && (
          <PlanListeDel>
            <VStack gap="space-16">
              {gjeldendeHendelser.map(({ organization, hendelse }) =>
                hendelse.type === "FERDIGSTILT_PLAN" ? (
                  <AktivPlanLinkCard
                    ...
                    aktivPlan={hendelse}
                    linkCardTitle={
                      organization.orgName ?? organization.orgNumber
                    }
        ...
        {harFerdigstiltePlaner && (
          <InlineMessage status="info" className="mt-4">
            Aktive og tidligere oppfølgingsplaner blir utilgjengelige når du
            ikke har hatt sykmelding hos arbeidsgiveren på 6 måneder. Åpne
            planen og velg «Vis PDF» for å lagre en kopi.
          </InlineMessage>
        )}
      </section>

      {erITiltaksgruppe && !visForberedelseForTomtilstand && (
        <ForberedelseTilSamtale harGjeldendeUnntak={harGjeldendeUnntak} />
      )}
    </>
  );
}
